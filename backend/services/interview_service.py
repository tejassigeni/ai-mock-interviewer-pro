...
async def submit_answer(db: AsyncSession, question_id: str, user_answer: str) -> Answer:
    ...
    try:
        ...
        if all_answered:
            ...
            try:
                await _update_analytics(db, interview.user_id, interview.total_score, len(interview.questions))
            except Exception as e:
                print(f"Analytics update failed (non-fatal): {e}")
    except Exception as e:
        print(f"Interview completion check failed (non-fatal): {e}")
...
async def _update_analytics(db: AsyncSession, user_id: str, interview_score: float, questions_count: int = 5):
    ...
    # Store previous count for average calculation
    prev_count = analytics.interviews_taken
    analytics.interviews_taken = prev_count + 1
    analytics.total_questions_answered += questions_count
    analytics.last_interview = datetime.now(timezone.utc)

    # Recalculate average using previous count
    if prev_count == 0:
        analytics.average_score = interview_score
    else:
        total = analytics.average_score * prev_count + interview_score
